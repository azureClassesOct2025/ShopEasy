...
3. **LogViewerService** - Reads and displays `backupLog` files written by NameInputService
...
### LogViewerService (Port: 5090)
- **Purpose**: View log files generated by NameInputService
- **API Endpoints**:
  - `GET /api/logs` - Lists available log files
  - `GET /api/logs/{filename}` - Returns contents of a log file
- **UI**: Accessible at `http://localhost:5090` (lists files and shows contents)
- **Configuration**:
  - `LogDirectory` (optional): Override the folder to read logs from
...
2. **Run both services**:
   ...
   **Terminal 3 - LogViewerService:**
   ```bash
   cd LogViewerService
   dotnet run
   ```
   The service will start at `http://localhost:5090`
...
2. Open the apps:
   ...
   - LogViewerService: `http://localhost:5090`
...
- `LogViewerService` mounts the host folder `NameInputService/bin/Debug/net8.0/backupLog` into `/logs` to display logs when using Docker Compose.
